...
if (count($navItems) > 0) {
    echo '<ul class="nav navbar-nav ml-auto">'; //opens the top-level menu

    foreach ($navItems as $ni) {
        $active = $ni->isCurrent ? 'active' : '';
        echo '<li class="' . $ni->classes . ' nav-item ' . $active . '">'; //opens a nav item
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . ' nav-link">' . h($ni->name) . '</a>';
